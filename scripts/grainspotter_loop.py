...
while testCon < testLim:
	...
	# line below here may have to be rewritten to work on Windows?
	command = "cat %s | sed s/_EULERSTEP_/%f/1 | sed s/_NSIGMAS_/%f/1 > grainspotter_current.ini" %(INIFILE,eulerstep,nsigmas)
	os.system(command)
	...
	shututil.move("spotter_out.gff",gff_out)
	shututil.move("spotter_out.log",log_out)
